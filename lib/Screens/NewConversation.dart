...
class NewConversation extends StatefulWidget {
  ...
  NewConversation(this.userId, this.sellerId);
  @override
...
class _NewConversationState extends State<NewConversation> {
  ...
  void initState() {
    ...
    print("u: ${widget.userId}, s: ${widget.sellerId}");
    GetChatMsg();
  }
    ...
    try {
      ...
      if (result.isNotEmpty && result[0].rawAddress.isNotEmpty) {
        FormData data = FormData.fromMap(
            {"userid": widget.userId, "sellerid": widget.sellerId});
        AppServices.GetUsertosellerChatMsg(data).then((data) async {
          setState(() {
            isLoading = false;
          });
          if (data.data == "0") {
  ...
  _sendMessage() async {
    ...
    try {
      ...
      if (result.isNotEmpty && result[0].rawAddress.isNotEmpty) {
        ...
        print("s: ${edtMessage.text} ${widget.userId} ${widget.sellerId}");

        AppServices.SendUsertosellerMessage(d).then((data) async {
          pr.hide();
          if (data.data == "0") {
